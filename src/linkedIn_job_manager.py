...
class LinkedInJobManager:
    ...
    def set_parameters(self, parameters):
        self.company_blacklist = parameters.get('companyBlacklist', [])
        self.title_blacklist = parameters.get('titleBlacklist', [])
    ...
    def start_applying(self):
        ...
        for position, location in searches:
            ...
            try:
                ...
                utils.printred(f"Error occurred: {traceback.format_exc()}")
                pass
            time_left = minimum_page_time - time.time()
            if time_left > 0:
                utils.printyellow(f"Sleeping for {time_left} seconds.")
    ...
    def apply_jobs(self):
        ...
        for job in job_list:
            ...
            try:
                ...
                utils.printred(f"Error applying for job: {traceback.format_exc()}")
    ...
    def extract_job_information_from_tile(self, job_tile):
        ...
        try:
            ...
            apply_method = 'Apply'
        return job_title, company, job_location, apply_method, link

    def is_blacklisted(self, title, company, link):
        if company in self.company_blacklist or title in self.title_blacklist:
            return True
        if self.env_config.skip_apply and link in self.seen_jobs:
            return True
        return False
