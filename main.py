...
class ConfigValidator:
    ...
    def validate_secrets(secrets_yaml_path: Path) -> tuple:
        ...
        mandatory_secrets = ['email', 'password', 'openai_api_key']  # Removed Gemini API key
...
def create_and_run_bot(email: str, password: str, parameters: dict, openai_api_key: str):
    try:
        ...
        raise RuntimeError(f"WebDriverException: {str(e)}")
    except TimeoutException as e:
        raise RuntimeError(f"TimeoutException: {str(e)}")
    except Exception as e:
        raise RuntimeError(f"An error occurred: {str(e)}")

@click.command()
@click.argument('app_data_folder', type=click.Path(exists=True, file_okay=False))
def main(app_data_folder: str):
    try:
        app_data_folder_path = Path(app_data_folder)
        secrets_yaml_path, config_yaml_path, plain_text_resume_yaml_path, output_folder = FileManager.validate_data_folder(app_data_folder_path)

        secrets_email, secrets_password, openai_api_key = ConfigValidator.validate_secrets(secrets_yaml_path)
        parameters = ConfigValidator.validate_config(config_yaml_path)

        resume_file = FileManager.find_file(name_containing='resume', with_extension='.pdf', at_path=app_data_folder_path)
        file_paths = FileManager.file_paths_to_dict(resume_file, plain_text_resume_yaml_path)

        create_and_run_bot(secrets_email, secrets_password, parameters, openai_api_key)
        print("Bot completed the job application process successfully.")
